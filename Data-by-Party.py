...
# download path
base_path = r"//10.30.31.77/data_collection_dump/RawData/Test UNFCCC/Data by Party" 
...
WebDriverWait(driver, 20).until(EC.presence_of_element_located((By.XPATH, '/html/body/div/div/div[1]/div/div/img')))
...
for option_country in options_countries:
    WebDriverWait(driver, 20).until(EC.element_to_be_clickable(option_country))
    ...
    for option_year in options_years:
        ...
        time.sleep(1)
        WebDriverWait(driver, 20).until(EC.element_to_be_clickable(option_year))
        option_year.click()
        for option_Total in options_Totals:
            WebDriverWait(driver, 20).until(EC.element_to_be_clickable(option_Total))
            option_Total.click()
            time.sleep(1)
            options_aggregates = select_elements[3].find_elements(By.TAG_NAME,'option')
            for option_aggregate in options_aggregates:
                WebDriverWait(driver, 20).until(EC.element_to_be_clickable(option_aggregate))
                ...
                for option_equivalent in options_equivalents:
                    WebDriverWait(driver, 20).until(EC.element_to_be_clickable(option_equivalent))
